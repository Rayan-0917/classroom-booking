...
const BookingGrid = ({ user, onBookingCreated }) => {
    ...
    const fetchRoomsAndBookings = async () => {
        ...
        try {
            ...
            if (fetchedRooms.length > 0) {
                if (!selectedRoom || !fetchedRooms.some(r => r.id === selectedRoom.id)) {
    ...
    return (
        <div>
            <div className='bg-white rounded-2xl border border-gray-50 shadow-sm'>
                <div className='flex flex-col p-6 bg-gray-50 md:flex-row md:items-center justify-between gap-4 mb-3 border-b border-gray-100'>
                    ...
                    <div className='flex flex-wrap items-center gap-3 mb-6'>
                        <div className='flex items-center bg-gray-50 rounded-xl p-1 border border-gray-200'>
                            ...
                            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className='bg-transparent font-semibold px-2 focus:outline-none cursor-pointer' />
                            <button onClick={() => shiftDate(1)} className='p-1.5 rounded-lg cursor-pointer'>
                ...
                <div className='my-6 mx-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50 rounded-xl border border-gray-100'>
                    <div className='flex items-center gap-3 w-full'>
                        <label htmlFor="room-select" className='font-semibold p-3 '>Select Room: </label>
                    </div>
                    <select id="room-select" value={selectedRoom ? selectedRoom.id : ""} onChange={handleRoomChange} disabled={loading || filteredRooms.length === 0} className='bg-white border border-gray-200 rounded-lg w-full font-medium p-2.5 mr-2'>
                        {filteredRooms.length === 0 ? (
                            <option value="">No rooms available</option>
                        ) : (
                            filteredRooms.map((room) => (
                                <option value={room.id} key={room.id}>Room {room.room_number} {room.is_high_priority ? "(High Priority)" : ""}</option>
                            ))
                        )}
                    </select>
                </div>
                {selectedRoom && (
                    <div className='flex items-center gap-3 p-5'>
                        <span>Capacity: <strong>{selectedRoom.capacity}</strong></span>
                        {selectedRoom.is_high_priority && (
                            <span className='bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full border border-amber-300 font-medium'>
                                High Priority Room
                            </span>
                        )}
                    </div>
                ...
                <div className='p-6'>
                    {loading ? (
                        ...
                        !selectedRoom ? (
                            <div className='p-12 text-center font-medium'>No rooms selected</div>
                        ) : (
                            <div className='border border-gray-100 rounded-xl p-6 bg-white shadow-sm'>
                                <div className='flex flex-wrap gap-2'>
                                    {time_slots.map((slot) => {
                                        const booking = getSlotDetails(selectedRoom.id, slot.timeStr);
                                        const isMine = booking && booking.user_id === user.id;
                                        const isPending = booking && booking.status === "Pending";
                                        const isSelected = selectedRoom && selectedRoom?.id === selectedRoom.id && selectedSlots?.some((s) => s?.id === slot.id)
                                        ...
                                        else if (selectedRoom.is_high_priority) {
                                            style = "bg-amber-400 border-amber-500 hover:bg-amber-500 cursor-pointer"
                                        }

                                        return (
                                            <button onClick={() => handleSlotSelection(selectedRoom, slot)} key={slot.id} className={`px-3 py-2 rounded-xl border font-medium transition flex flex-col items-center justify-center min-w-19 ${style}`}>
                    ...
                    {selectedRoom && selectedSlots.length > 0 && (
                        <div className='mt-6 bg-gray-200 p-4 rounded-xl shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4'>
                            <div>
                                <div className='font-medium'>
                                    Room {selectedRoom.room_number} | {selectedDate}
                                </div>
                                <div className='font-medium'>
                                    Time: {getSelectedSlotsRange()}
                                </div>
                            </div>
