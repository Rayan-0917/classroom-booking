...
const getPendingApprovals=async (req, res)=>{
    try {
        const res=await pool.query("SELECT b.id AS booking_id, b.start_time, b.end_time, b.created_at, r.id AS room_id, r.room_number, u.id AS user_id, u.name, u.email FROM booking b JOIN rooms r ON b.room_id=r.id JOIN users u ON b.user_id=u.id WHERE b.status='Pending' AND r.is_high_priority=TRUE ORDER BY created_at ASC")

        res.json({pendingApprovals: res.rows});
    } catch (error) {
        console.log(error);
    }
}
...
const getPendingReassignments=async(req, res)=>{
    try {
        const query=await pool.query(`SELECT re.id as reassignment_id, re.status, re.created_at, b.id as original_booking_id, b.start_time, b.end_time, r.room_number, previous_u.name as previous_faculty_name, previous_u.email as previous_faculty_email, new_u.name as new_faculty_name 
...
const reassignRoom=async(req, res)=>{
    ...
    try {
        ...
        const newBooking=await client.query("INSERT INTO bookings (room_id, user_id, start_time, end_time, status) VALUES ($1, $2, $3, $4, $5) RETUNING *", [new_room_id, user_id, start_time, end_time, 'Approved']);

        await client.query("UPDATE reassignents SET new_room_id=$1, status=$2 WHERE id=$3", [new_room_id, 'Resolved', reassignment_id]);
