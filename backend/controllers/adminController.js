...
const getPendingApprovals=async (req, res)=>{
    try {
        const result=await pool.query("SELECT b.id AS booking_id, b.start_time, b.end_time, b.created_at, r.id AS room_id, r.room_number, u.id AS user_id, u.name, u.email FROM bookings b JOIN rooms r ON b.room_id=r.id JOIN users u ON b.user_id=u.id WHERE b.status='Pending' AND r.is_high_priority=TRUE AND b.start_time > NOW() ORDER BY created_at ASC")
...
const HandlePendingBookings=async(req, res)=>{
    ...
    try {
        const result=await pool.query("UPDATE bookings SET status=$1 WHERE id=$2 AND status='Pending' RETURNING *", [decision, id]);
...
const getPendingReassignments=async(req, res)=>{
    try {
        const query=`SELECT re.id as reassignment_id, re.status, re.created_at, b.id as original_booking_id, b.start_time, b.end_time,r.id as original_room_id, r.room_number, previous_u.name as previous_faculty_name, previous_u.email as previous_faculty_email, new_u.name as new_faculty_name, new_u.email as new_faculty_email 
...
const reassignRoom=async(req, res)=>{
    ...
    try {
        ...
        const result=await client.query("SELECT re.*, b.user_id, b.start_time, b.end_time FROM reassignments re JOIN bookings b ON re.previous_booking_id=b.id WHERE re.id=$1 AND re.status='Pending' FOR UPDATE", [reassignment_id]);
        ...
        const {user_id, start_time, end_time}=result.rows[0];
        ...
        await client.query("UPDATE reassignments SET new_room_id=$1, status=$2 WHERE id=$3", [new_room_id, 'Resolved', reassignment_id]);
